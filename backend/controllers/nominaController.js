...
export const getNominas = async (req, res) => {
  try {
    ...
    query += " ORDER BY n.FechaProceso DESC, e.Nombres";
...
const calcularNominaEmpleado = async (pool, empleadoId, periodo) => {
  try {
    ...
    const empleadoResult = await pool.request()
      ...
      .query('SELECT * FROM Empleados WHERE EmpleadoId = @empleadoId AND Estado = \'Activo\'');
    ...
    incidencias.forEach(incidencia => {
      switch (incidencia.Tipo) {
        case 'horas_extra':
          const valorHora = salarioBase / 184; // Horas mensuales aproximadas
        ...
        case 'falta':
          if (!incidencia.Justificada) {
            descuentoFaltas += salarioBase / 30; // Valor día
          }
          break;
...
export const procesarNomina = async (req, res) => {
  ...
  try {
    ...
    let empleadosQuery = 'SELECT EmpleadoId FROM Empleados WHERE Estado = \'Activo\'';
    const request = pool.request();
    ...
    for (const empleado of empleados) {
      ...
      // Insertar registro principal de nómina
      const nominaRequest = pool.request()
        .input('empleadoId', sql.Int, empleado.EmpleadoId)
        .input('periodo', sql.VarChar, periodo)
        .input('fechaProceso', sql.DateTime, new Date())
      ...
      const nominaResult = await nominaRequest.query(`
        INSERT INTO Nomina (
          EmpleadoId, Periodo, FechaProceso, Estado,
      ...
      // Insertar detalles de conceptos
      const conceptos = [
    ...
    switch (tipo) {
      ...
      default: // completo
        query = `
          ...
          INNER JOIN Empleados e ON n.EmpleadoId = e.EmpleadoId
          ${periodo ? 'WHERE n.Periodo = @periodo' : ''}
          ORDER BY n.Periodo DESC, e.Nombres
        `;
    }
